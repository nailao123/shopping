      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index)item in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
            ...
            <div class="addr-opration addr-set-default">
              <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.addressId)"><i>是否设置为默认地址</i></a>
          </div>
...
export default{
  ...
  methods:{
    /**
     *初始化地址方法
     *axion.get()方法获取数据库中的地址addressList
     *
     *
     */
    ...
    /**
     *定义展开方法
     *就是判断limit的长度是否为3  如果是表示没有展开  将原有数组长度赋值给它
     * 如果不等于三 则为展开状态
     *
     */
    expand(){
